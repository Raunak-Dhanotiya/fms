...
export class TeamComponent implements OnInit {
  ...
  loadEnums(){
    ...
    this.enumsrv.getEnums().subscribe(
      (res: EnumList[]) => {
        const enumList = Array.isArray(res) ? res : [];
        this.enumClonedList = enumList.map(x => Object.assign({}, x));
        ...
        const enumEmployee = this.enumTeamType.find(t => t.enumValue?.toLocaleUpperCase() === 'Employee'.toLocaleUpperCase());
        this.enumEmployeeType = enumEmployee?.enumKey ?? null;
        this.loadRecords();
      },
      _error => {
        this.enumTeamType = [];
  ...
  loadRecords() {
    ...
    this.teamService.getAllTeamsPaginated(data).subscribe((res: any) => {
      if (res.status != 202) {
        ...
        if(!this.showType && this.enumEmployeeType){
