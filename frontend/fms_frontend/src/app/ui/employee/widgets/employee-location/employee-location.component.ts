...
export class EmployeeLocationComponent implements ControlValueAccessor, OnDestroy {
  ...
  set value(value: any) {
    const location = value?.em ?? value;
    if (!location) {
      return;
    }
    if (location.blId != null) {
      const bl = location.bl ?? {};
      const blData: any = {
        blId: location.blId,
    ...
    if (location.flId != null) {
      const bl = location.bl ?? {};
      const fl = location.fl ?? {};
      const flData: any = {
        flId: location.flId,
    ...
    if (location.rmId != null) {
      const rm = location.rm ?? {};
      const rmData = {
        rmId: location.rmId,
    ...
    setTimeout(() => {
      ...
      this.frmEmployeeLocation.patchValue({
        divId: location.divId,
      });
      this.loadDepartmentCode(location.divId);
      this.loadSubDepartmentCode(location.divId, null);
      this.frmEmployeeLocation.patchValue({
        depId: location.depId,
      });
      if (location.depId != null && location.divId != null) {
        this.loadSubDepartmentCode(location.divId, location.depId);
      }
      this.frmEmployeeLocation.patchValue({
        subDepId: location.subDepId,
      });
      this.work_type = location.workType != null ? location.workType.trim() : "";
  ...
  loadCommonAreaNoneEnum() {
    this.enumservice.getEnums().subscribe((res: any) => {
      const enums = Array.isArray(res) ? res : [];
      let commonAreaNoneList = enums.filter((t: any) =>
      ...
      this.enumCommonAreaTypeNone = commonAreaNoneList[0]?.enumKey ?? 0;
    })
  }
