...
export class SupervisorDashboardComponent {
  ...
  loadEnums() {
    ...
    this.enumsrv.getEnums().subscribe(
      (res: EnumList[]) => {
        ...
        this.enumStatus = this.enumClonedList.filter(t => (t.tableName?.toUpperCase() || '') === 'WR' && (t.fieldName?.toUpperCase() || '') === 'STATUS');
      }
    )
  }
  ...
  loadAppParamsById() {
    this.appParamsServ.getAppParamByCode('financial_year').subscribe((res: any) => {
      ...
      // ensure we have a date range and then load work requests
      this.ensureDefaultDateRangeIfMissing();
      this.loadWrRecords();
    }, _err => {
      // on error also fallback to default range and load
      this.ensureDefaultDateRangeIfMissing();
      this.loadWrRecords();
    })
  }

  // If app param is missing or malformed, fallback to last 30 days by default
  private ensureDefaultDateRangeIfMissing() {
    if (!this.dateRequestedFrom || !this.dateRequestedTo) {
      const today = new Date();
      const prior = new Date();
      prior.setDate(today.getDate() - 30);
      const to = this.datePipe.transform(today, 'yyyy-MM-dd');
      const from = this.datePipe.transform(prior, 'yyyy-MM-dd');
      this.dateRequestedFrom = from;
      this.dateRequestedTo = to;
    }
  }
  ...
  filterByStatus() {
    ...
    setTimeout(() => {
      const requestData = this.allRequestData ?? [];
      this.statusChartData.requested.data = requestData.filter(wr => wr.status === this.getIdByStatus('Requested'));
      this.statusChartData.approved.data = requestData.filter(wr => wr.status === this.getIdByStatus('Approved'));
      this.statusChartData.rejected.data = requestData.filter(wr => wr.status === this.getIdByStatus('Rejected'));
      this.statusChartData.onHoldForAccess.data = requestData.filter(wr => wr.status === this.getIdByStatus('On Hold For Access'));
      this.statusChartData.onHoldForParts.data = requestData.filter(wr => wr.status === this.getIdByStatus('On Hold For Parts'));
      this.statusChartData.onHoldForLabour.data = requestData.filter(wr => wr.status === this.getIdByStatus('On Hold For Labour'));
      this.statusChartData.inProcess.data = requestData.filter(wr => wr.status === this.getIdByStatus('In Process'));
      this.statusChartData.completed.data = requestData.filter(wr => wr.status === this.getIdByStatus('Completed'));
      this.statusChartData.close.data = requestData.filter(wr => wr.status === this.getIdByStatus('Close'));
      this.statusChartData.cancelled.data = requestData.filter(wr => wr.status === this.getIdByStatus('Cancelled'));
    }, 0);
  ...
  filterByEscaltion() {
    ...
    setTimeout(() => {
      ...
      (this.allRequestData ?? []).forEach((item: any) => {
        ...
        if (!isEscalatedForResponse && !isEscalatedForComplete && (this.getValueById(status)?.toUpperCase() || '') !== 'CANCELLED') {
      ...
      this.noEscalatedPercent = this.allEscalatedData.length > 0 ? ((this.noEsctdData.length / this.allEscalatedData.length) * 100).toFixed(2) : '0.00';  // upto two decimal
    }, 0);
  ...
  getIdByStatus(status: any) {
    ...
    return this.enumStatus.find((t: any) => (t.enumValue?.toUpperCase() || '') === normalizedStatus)?.enumKey;
  };
