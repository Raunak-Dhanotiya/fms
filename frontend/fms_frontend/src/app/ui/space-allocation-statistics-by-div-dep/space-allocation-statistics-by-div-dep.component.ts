...
export class SpaceAllocationStatisticsByDivDepComponent {
  ...
  loadAllDivisions() {
    ...
    this.divisionservice.getAllDivisions().subscribe((res: any) => {
      this.enumDivision = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
  ...
  loadAllDepartments() {
    ...
    this.departmentservice.getAllDepartments().subscribe((res: any) => {
      this.alldepartmentData = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
  ...
  loadSubDepartment() {
    ...
    this.subDeptService.getAllSubDepartments().subscribe((res: any) => {
      const subDepartments = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
      if (subDepartments.length > 0) {
        this.enumAllSubDepartment = subDepartments;
  ...
  loadAllBuilding() {
    this.blServ.getALLBuilding().subscribe((res: any) => {
      this.enumBL = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
  ...
  loadAllFloor() {
    this.blServ.getALLFloor().subscribe((res: any) => {
      this.enumAllFL = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
  ...
  loadCommonAreaEnums() {
    this.enumsrv.getEnums().subscribe((res: any) => {
      const enums = Array.isArray(res) ? res : (res?.content ?? res?.data ?? []);
      let resultCommonArea = enums.filter((t: any) =>
      ...
      this.enumCommonAreaNoneType = resultCommonArea[0]?.enumKey ?? 0
    })
  }
