  ...
  loadUserProfile(user_name: string) {
    this.userSrv.getUserProfile(user_name).subscribe((res: UserProfile) => {
      if (res) {
        ...
        // guard against null/undefined emPhoto
        this.isInitialShow = !(res.emPhoto && res.emPhoto.length > 0);
        if (!this.isInitialShow && res.emPhoto)
  ...
  userPhotoChanged() {
    this.emSrv.getEmById(this.userProfileRec.emEmployeeEmId).subscribe((res) => {
      if (res && res.employeeDetails) {
        const mobilePhoto = res.employeeDetails.emPhotoMobile;
        this.isInitialShow = !(mobilePhoto && mobilePhoto.length > 0);
        if (!this.isInitialShow && mobilePhoto)
          this.userProfilePic = "data:image/png;base64," + mobilePhoto;
      }
    })
  }
