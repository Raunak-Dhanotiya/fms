...
export class AddUserComponent implements ControlValueAccessor, OnDestroy {
 ...
 @Input() isNew!:boolean;
  ...
  set value(value:UserOutput ) { 
      setTimeout(() => {
        this.userFormPanel.patchValue({
          ...
          userId: value.userId,
          userStatus: this.normalizeSelectValue(value.userStatus),
          userRoleId: this.normalizeSelectValue(value.userRoleId),
          deviceId: value.deviceId,
          ipAddress: value.ipAddress,
          emId: this.normalizeEmployeeValue(value.emId),
          compId:value.compId,
          technicianId: this.normalizeSelectValue(value.technicianId),
        });
  ...
  isCreateEmp(em_id:any){
    if(this.hasSelectedEmployee(em_id)){
      this.isCreateNewEmployeeChkBox = false;
      this.isCreateNewEmployee = false;
  ...
  private hasSelectedEmployee(em_id: any): boolean {
    return em_id != null && em_id !== '' && em_id !== '0' && em_id !== 0;
  }

  private normalizeSelectValue(value: any): any {
    return value === '' || value === undefined ? null : value;
  }

  private normalizeEmployeeValue(value: any): any {
    return this.hasSelectedEmployee(value) ? value : null;
  }
  loadUnAssignEmployee(emId:any):void{
    this.emSrv.getAllUnAssignEmployees(emId).subscribe((res:any)=>{
      this.enumEmp = Array.isArray(res) ? res : (res?.items ?? res?.content ?? res?.data ?? []);
   //   this.enumEmp =  this.enumEmp.map(i=>{i.name = i.emCode + " - "+ i.name; return i;});

      if (Array.isArray(this.enumEmp)) {
        this.enumEmp.unshift({id:null,  name:'Make a selection'});
      } else {
        this.enumEmp = [{id:null,  name:'Make a selection'}];
      }
    })
  }
  ...
  loadUnAssignedTechnician(technicianId: any){
    this.technicianServ.getUnAssignedTechnician(technicianId).subscribe((res: any)=>{
      const technicians = Array.isArray(res) ? res : (res?.items ?? res?.content ?? res?.data ?? []);
      if (Array.isArray(technicians)) {
        this.technicianData = technicians;
        this.technicianData.unshift(new Craftsperson(null, "Make a selection", "", "", 0, 0, 0, "", 0, 0, 0, 0, 0, ""));
      } else {
        this.technicianData = [new Craftsperson(null, "Make a selection", "", "", 0, 0, 0, "", 0, 0, 0, 0, 0, "")];
      }
    })
  }
}
